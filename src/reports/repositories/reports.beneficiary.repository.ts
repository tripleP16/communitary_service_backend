...
export class ReportsBeneficiaryRepository {
    ...
    async getBeneficiariesReport(dto: GetBeneficiariesReportDtoToService) {
        ...
        const existingBeneficiaries = await this._beneficiariesModel.find({
        ...
        if (existingBeneficiaries.length !== beneficiariesIds.length) {
            ...
            const missingIds = beneficiariesIds.filter(id => !existingIds.includes(id));
            throw new NotFoundException(`Beneficiaries not found: ${missingIds.join(', ')}`);
        }
        ...
        return results.map(result => {
            const beneficiary = existingBeneficiaries.find(b => b._id.toString() === result.beneficiaryId.toString());
            return {
            ...
            case ReportType.bmi:
                return {
                    $divide: [
                        ...
                        {
                            $pow: [
                                { $divide: ["$medicalHistories.height", 100] },
                                2,
                            ],
                        },
                    ],
